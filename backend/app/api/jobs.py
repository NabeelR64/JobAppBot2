...
def get_recommendations(
    ...
    """
    # ⚡ Bolt: Use a subquery for swiped_ids instead of fetching them into memory.
    # Why: Passing a massive list of IDs to notin_() causes O(N) application memory usage
    # and massive SQL IN clauses. By leaving it as a Query object, SQLAlchemy generates
    # an efficient "NOT IN (SELECT ...)" subquery.
    # Impact: O(1) memory usage in Python, faster query execution time, fixes N+1 scaling issue.
    swiped_ids_subquery = db.query(SwipeAction.job_posting_id).filter(
        SwipeAction.user_id == current_user.id
    )

    jobs = (
        db.query(JobPosting)
        .filter(JobPosting.id.notin_(swiped_ids_subquery))
        .offset(skip)
        .limit(limit)
        .all()
    )
    ...
    if not jobs:
        ...
        # Query again using the same subquery
        jobs = (
            db.query(JobPosting)
            .filter(JobPosting.id.notin_(swiped_ids_subquery))
            .offset(skip)
...
def swipe_job(
    ...
    # Record swipe
    action = SwipeAction(
        user_id=current_user.id, job_posting_id=job_id, action=swipe.direction
    )
    db.add(action)
