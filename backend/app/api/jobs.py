...
def get_recommendations(
    ...
    """
    # Get swiped job ids as a subquery instead of a list
    # ⚡ BOLT OPTIMIZATION: Using a subquery directly prevents massive O(N) memory allocation
    # for users with many swipes and avoids generating SQL queries with huge IN clauses
    swiped_ids_query = db.query(SwipeAction.job_posting_id).filter(
        SwipeAction.user_id == current_user.id
    )

    jobs = (
        db.query(JobPosting)
        .filter(JobPosting.id.notin_(swiped_ids_query))
        .offset(skip)
        .limit(limit)
        .all()
    )
